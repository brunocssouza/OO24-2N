...
export function torneioTriBruxoIntro() {
    while (true) {
        ...
        const criaturas = CriaturaMagica.getAll()
        switch(opcao) {
            case 0:
                return;

            case 1:
                const apenasBruxos = criaturas.filter(c => c instanceof Bruxo);
                torneioTriBruxo(apenasBruxos);
                return

                case 2:
                    const bruxos_e_criaturas = criaturas.filter(c => !(c instanceof Guardiao));
